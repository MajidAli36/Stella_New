...
interface Iprops {
    kidId: string | undefined, houseId: string | undefined
}
...
export default function KidFileTab(props: Iprops) {
    ...
    const downloadFile = (filename: string, filePath: string) => {
        ...
        const path = constants.House_Files + filePath;
        fetch(path)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    const getCurrentPageFiles = (): any[] => {
        const startIndex = (currentPage - 1) * PAGE_SIZE;
        const endIndex = startIndex + PAGE_SIZE;
        return (fileList || []).slice(startIndex, endIndex);
    };
    const getFileList = () => {
        if (props.kidId != null && props.kidId != undefined) {
            GetAxios().get(constants.Api_Url + 'File/GetKidFiles?kidId=' + props.kidId).then(res => {
                if (res.data.success) {
                ...
                {fileList != undefined && fileList?.length > 0 &&
                    <>
                        <Grid className='mb-4' container spacing={5}>
                            {(getCurrentPageFiles() || []).map((item: FileListModel, index: any) => {
                                return (
                        ...
                        <div className="d-flex align-items-center w-100 justify-content-between">
                            <Stack spacing={2}>
                                <Pagination count={Math.ceil((fileList?.length || 0) / PAGE_SIZE)} page={currentPage} onChange={handlePageChange} />

                            </Stack>
                            <div>
                                <DisplayNumber>
                                    Displaying {getCurrentPageFiles.length} of {fileList?.length} Files
                                </DisplayNumber>
                            </div>
                        </div>
                    </>
